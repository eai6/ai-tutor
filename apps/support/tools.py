...
def recommend_next_lesson(user, *, course_query: Optional[str] = None) -> Dict:
    """Recommend the next lesson for a student. Read-only.

    Mirrors the catalog's recommendation logic — picks the first
    unmastered lesson in the most recently progressed course.
    """
    if not user or not user.is_authenticated:
        return {'ok': False, 'human_msg': 'You need to be signed in.'}
    ...
    if course_query:
        ...
        lessons_qs = Lesson.objects.filter(is_published=True).order_by(
            'unit__course__title', 'unit__order_index', 'order_index',
        )

    mastered_ids = set(
        StudentLessonProgress.objects.filter(
            student=user, mastery_level='mastered',
        ).values_list('lesson_id', flat=True)
    )
    for lesson in lessons_qs[:50]:
        if lesson.id not in mastered_ids:
            return {
                'ok': True,
                'lesson': {
                    'id': lesson.id,
                    'title': lesson.title,
                    'objective': (lesson.objective or '')[:200],
                    'course_title': lesson.unit.course.title,
                    'url': f'/tutor/lesson/{lesson.id}/',
                },
            }
    return {'ok': True, 'lesson': None,
            'human_msg': "You've mastered everything we have published. Nice work."}
...
def assign_lesson_for_week(user, *, course_id: int, lesson_id: int,
                           week_start: str, notes: str = '') -> Dict:
    """Create or update a WeeklyAssignment for a course + week.

    Permission re-checked: caller must manage the course. Lesson
    must belong to the course. Date must parse as YYYY-MM-DD.
    """
    if not _is_staff(user):
        return {'ok': False, 'human_msg': 'Teacher access required.'}
    ...
    try:
        ...
        return {'ok': False, 'human_msg': f"Invalid date '{week_start}'. Use YYYY-MM-DD."}

    from apps.curriculum.models import Course, Lesson
    course = Course.objects.filter(id=course_id).first()
    if not course or not _can_manage_course(user, course):
        return {'ok': False, 'human_msg': 'You cannot manage that course.'}
    lesson = Lesson.objects.filter(id=lesson_id, unit__course=course).first()
    if not lesson:
        return {'ok': False, 'human_msg': 'That lesson is not in that course.'}

    try:
        from apps.dashboard.models import WeeklyAssignment
    except ImportError:
        return {'ok': False, 'human_msg': 'Weekly assignments not available on this deploy.'}
    wa, _ = WeeklyAssignment.objects.update_or_create(
        course=course, week_start=week_dt,
        defaults={'notes': (notes or '')[:500]},
    )
    wa.lessons.add(lesson)
    return {
        'ok': True,
        'human_msg': f"Added '{lesson.title}' to week of {week_dt} for {course.title}.",
        'assignment_id': wa.id,
    }


def set_default_lesson_duration(user, *, course_id: int, minutes: int) -> Dict:
    """Bulk-update Lesson.estimated_minutes for all lessons in a
    course. No regen — duration is a runtime knob now."""
    if not _is_staff(user):
        return {'ok': False, 'human_msg': 'Teacher access required.'}
    if not (5 <= int(minutes) <= 120):
        return {'ok': False, 'human_msg': 'Duration must be 5–120 minutes.'}
    from apps.curriculum.models import Course, Lesson
    course = Course.objects.filter(id=course_id).first()
    if not course or not _can_manage_course(user, course):
        return {'ok': False, 'human_msg': 'You cannot manage that course.'}
    n = Lesson.objects.filter(unit__course=course).update(estimated_minutes=int(minutes))
    return {
        'ok': True,
        'human_msg': f"Set default duration to {minutes} min for {n} lesson(s) in {course.title}.",
    }
...
# `requires_confirmation=True` means writes — UI shows a Confirm/Cancel card.
_CATALOG = [
    {
        'name': 'find_help_doc',
    ...
    },
    {
        ...
        'description': 'Recommend the next lesson the student should do, optionally scoped to a specific course.',
        'input_schema': {
            'type': 'object',
            'properties': {
                'course_query': {'type': 'string', 'description': 'Optional course title or fragment.'},
            },
        },
        'audience': 'student',
        'requires_confirmation': False,
        'handler': recommend_next_lesson,
    },
    {
        'name': 'start_lesson',
    ...
    },
    {
        ...
        'description': 'Assign a lesson to a specific week for a course. Creates / updates a WeeklyAssignment row.',
        'input_schema': {
            'type': 'object',
            'properties': {
            ...
            'required': ['course_id', 'lesson_id', 'week_start'],
        },
        'audience': 'staff',
        'requires_confirmation': True,
        'handler': assign_lesson_for_week,
    },
    {
        'name': 'set_default_lesson_duration',
        'description': 'Set the default lesson duration (in minutes) for every lesson in a course. No regen — runtime knob only.',
        'input_schema': {
            'type': 'object',
            'properties': {
                'course_id': {'type': 'integer'},
            ...
            'required': ['course_id', 'minutes'],
        },
        'audience': 'staff',
        'requires_confirmation': True,
        'handler': set_default_lesson_duration,
    },
]
