...
5. NEVER pretend to show images that don't exist - only use available media below
6. Use [SESSION_COMPLETE] ONLY when exit ticket is passed (8/10+)
...
"""
    ...
    def _format_media_list(self) -> str:
        ...
        if not self.lesson_media:
            return "None - do not reference any images"
...
"""
                    ...
                    media_instructions = """
No pre-made images available. If a visual would help, use:
[GENERATE_IMAGE:description of educational diagram needed]
"""
...
2. IMMEDIATELY show a relevant image using [ARTIFACT:media] with exact title from available media
...
Remember: SHOW IMAGES! They help students understand better.
"""
            else:
...
"""
    ...
    def _check_answer(self, question: Dict, answer: str) -> bool:
        ...
        # Simple match
        if answer.strip().upper() == correct.strip().upper():
            return True
        
        # Letter match (A, B, C, D)
        if len(answer.strip()) == 1 and answer.strip().upper() == correct.strip().upper():
            return True
