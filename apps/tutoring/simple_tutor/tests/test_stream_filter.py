...
from apps.tutoring.simple_tutor.stream_filter import (
...
class OllamaStreamReassemblyTest(SimpleTestCase):
    ...
    def _client(self):
        from apps.llm.client import OllamaClient
        cfg = SimpleNamespace(
    ...
    def test_tool_calls_are_collected_for_the_adapter(self):
        ...
        from apps.llm.client import _adapt_ollama_response
...
class StreamGateRotationTest(DjangoTestCase):
    ...
    def test_rotation_index_is_consumed_exactly_once(self):
        ...
        with patch('apps.tutoring.simple_tutor.engine._rotation_index',
                   return_value=0) as m:
...
class CallModeTest(SimpleTestCase):
    ...
    def test_auto_keeps_anthropic_on_two_calls(self):
        import os
        from apps.tutoring.simple_tutor.engine import _call_mode
        with patch.dict(os.environ, {}, clear=False):
