...
| 2 | **Direct Instruction + Active Practice** | LessonSteps serve as guidance, and the tutor is instructed to scaffold. | The tutor is described as "Socratic" — but pure Socratic questioning without explicit instruction is closer to "Active and Unguided," which the research ranks below "Active and Direct." The system prompt should explicitly mandate *direct explanation of the method first*, then Socratic questioning to check understanding and guide practice. |
| 3 | **Deliberate Practice** | `practice_correct` / `practice_total` tracked in engine state. `student_struggles` and `student_strengths` arrays maintained. | Practice difficulty is **not adapted** within a session. All students see the same lesson steps as guidance. The tutor has no mechanism to select easier or harder variants based on `student_struggles`. The struggle/strength arrays are populated but not fed back into LLM prompting to adjust difficulty. |
...
| 5 | **Minimising Cognitive Load** | Responses capped at 2–4 sentences. Media integration exists (`_find_matching_media`, `_generate_visual_aid`). `LessonStep.educational_content` holds worked examples. | `educational_content.worked_example` is **not explicitly surfaced** in the LLM context — the tutor receives `teacher_script` as guidance, and whether it includes the worked example depends on content generation. There is no instruction to the LLM to present worked examples before practice with labelled subgoals. Media is offered reactively ("offer diagrams when concepts are spatial/complex") rather than proactively integrated at the right instructional moment. |
...
| `StudentSkillMastery` + SM-2 | ✅ Built, ❌ `record_attempt()` never called | No spaced repetition happening |
| `SkillPracticeLog` | ✅ Built, ❌ Never written to | No learning analytics feeding back into adaptation |
