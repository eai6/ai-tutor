...
EXPOSE 8000
CMD ["sh", "-c", "python manage.py migrate && python manage.py seed_gamification && python manage.py backfill_progress && python manage.py classify_unit_grades && python manage.py seed_help_assistant_model && cp -r /app/media/vectordb /tmp/vectordb 2>/dev/null || true && python manage.py build_help_index --with-source && gunicorn config.wsgi:application --bind 0.0.0.0:8000 --workers 4 --threads 4 --timeout 120"]
