...
from ai_tutor.apps.tutoring.simple_tutor.tests.test_tools import _make_session
...
class DuplicatePoseQuestionTests(SimpleTestCase):
    ...
    def _dispatch(self, response):
        with patch('ai_tutor.apps.tutoring.simple_tutor.tools.handle_pose_question_by_index') as pose, \
             patch('ai_tutor.apps.tutoring.simple_tutor.tools.handle_record_answer') as rec:
...
class Call2FoldTests(SimpleTestCase):
    ...
    def _run(self, response, missing_tool, call2):
        ...
        with patch.object(engine, '_call_llm', return_value=call2) as call, \
             patch('ai_tutor.apps.tutoring.simple_tutor.tools.handle_pose_question_by_index') as pose, \
             patch('ai_tutor.apps.tutoring.simple_tutor.tools.handle_record_answer') as rec:
    ...
    def test_repair_rides_along_when_call1_fired_a_different_tool(self):
        ...
        with patch('ai_tutor.apps.tutoring.simple_tutor.tools.handle_record_answer') as rec:
            rec.return_value = {'recorded': True, 'verdict': 'correct'}
...
class ToolNameNormalisationTests(SimpleTestCase):
    ...
    def test_camelcase_name_is_normalised(self):
        ...
        with patch('ai_tutor.apps.tutoring.simple_tutor.tools.handle_request_figure') as fig:
