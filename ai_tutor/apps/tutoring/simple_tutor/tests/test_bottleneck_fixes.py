...
from ai_tutor.apps.tutoring.simple_tutor.tests.test_engine import _make_session
...
class DispatchOrderTest(DjangoTestCase):
    ...
    def _response(self, *, pose_stem, pose_ref, answer):
        ...
        from types import SimpleNamespace
        from ai_tutor.apps.tutoring.simple_tutor.tests.test_engine import _llm_response
    ...
    def _dispatch(self, session, response):
        from ai_tutor.apps.tutoring.simple_tutor.engine import _dispatch_tools
...
class PivotGuidanceTest(SimpleTestCase):
    ...
    def _block(self, attempts):
        from ai_tutor.apps.tutoring.simple_tutor.prompts import _render_in_flight_block
        slot = SimpleNamespace(
...
class AutoPoseFallbackTest(DjangoTestCase):
    ...
    def test_poses_next_pool_question_and_appends_text(self):
        from ai_tutor.apps.tutoring.simple_tutor.engine import _auto_pose_fallback
        from ai_tutor.apps.tutoring.simple_tutor.tests.test_engine import _make_session
        from ai_tutor.apps.curriculum.models import LessonStep
...
class QwenVariantCycle10Test(SimpleTestCase):
    ...
    def _qwen_block(self):
        from ai_tutor.apps.tutoring.simple_tutor.family_prompts import (
...
class DedupeReplyTest(DjangoTestCase):
    ...
    def test_identical_reply_is_varied(self):
        from ai_tutor.apps.tutoring.simple_tutor.engine import _dedupe_reply
        session, _ = _make_session()
...
class PolarityAlignTest(DjangoTestCase):
    ...
    def test_negative_opener_on_correct_verdict_replaced(self):
        from ai_tutor.apps.tutoring.simple_tutor.engine import _align_reply_polarity
        session, _ = _make_session()
...
class AutoGradeFallbackTest(DjangoTestCase):
    ...
    def test_grades_raw_answer_when_model_declined(self):
        from ai_tutor.apps.tutoring.simple_tutor.engine import _auto_grade_fallback
        session, _ = _make_session()
        self._slot(session)
        results = []
...
class RetryLadderTest(SimpleTestCase):
    ...
    def test_client_ladder_extended(self):
        from ai_tutor.apps.llm.client import _TRANSIENT_BACKOFF
        self.assertGreaterEqual(max(_TRANSIENT_BACKOFF), 30)

    def test_engine_ladder_extended(self):
        from ai_tutor.apps.tutoring.simple_tutor.engine import _TRANSIENT_BACKOFF
        self.assertGreaterEqual(max(_TRANSIENT_BACKOFF), 30)
...
class HardPivotTest(DjangoTestCase):
    ...
    def test_slot_replaced_at_four_attempts(self):
        from ai_tutor.apps.tutoring.simple_tutor.engine import _force_pivot_stuck_slot
        from ai_tutor.apps.curriculum.models import LessonStep
        session, _ = _make_session()
        step = LessonStep.objects.filter(lesson=session.lesson).first()
...
class RetryBudgetTest(SimpleTestCase):
    ...
    def test_error_detail_surfaces_the_response_body(self):
        ...
        from ai_tutor.apps.tutoring.simple_tutor.engine import _error_detail
...
class NarratedQuestionWithNoPoseTest(DjangoTestCase):
    ...
    def test_narrated_question_is_replaced_by_the_live_slot(self):
        from ai_tutor.apps.tutoring.simple_tutor.engine import _ensure_posed_question_in_text
        session, _ = _make_session()
        self._slot(session)
        text = (
...
class ToolNameNormalisationOrdersDispatchTest(DjangoTestCase):
    ...
    def test_canonical_name_grades_first(self):
        from ai_tutor.apps.tutoring.simple_tutor.engine import _pose_before_record
