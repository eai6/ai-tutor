...
def respond(
    ...
    from ai_tutor.apps.tutoring.simple_tutor.state import (
        build_recent_window, step_summary_log,
    )
    from ai_tutor.apps.tutoring.models import TutorSession as _TS
    ...
    # in an en-us course.
    from ai_tutor.apps.tutoring.simple_tutor.intent import classify_student_message
    ...
    from ai_tutor.apps.tutoring.simple_tutor.model_choice import resolve_for_session
    ...
    if on_delta is not None:
        from ai_tutor.apps.tutoring.simple_tutor.stream_filter import StreamGate
    ...
    from ai_tutor.apps.tutoring.simple_tutor.tools import maybe_complete_remediation
    ...
    from ai_tutor.apps.tutoring.simple_tutor.tools import maybe_pivot_stalled_question
    _pivoted = (
    ...
    from ai_tutor.apps.tutoring.simple_tutor.exit_ticket import maybe_pose_remediation_next
    _follow_up = (
...
def _auto_grade_fallback(
    *, session, family, student_intent, user_input, tool_results,
    ...
    if not InFlightQuestion.objects.filter(session=session).exists():
        return
    from ai_tutor.apps.tutoring.simple_tutor.tools import handle_record_answer
...
def _call_llm(
    *,
    ...
    if profile is not None:
        try:
            from ai_tutor.apps.tutoring.simple_tutor.prompts import family_prompt_delta
...
def _pose_before_record(session, tool_use_blocks, question_pool=None) -> bool:
    ...
    from ai_tutor.apps.tutoring.models import InFlightQuestion, SessionTurn
    if InFlightQuestion.objects.filter(session=session).exists():
