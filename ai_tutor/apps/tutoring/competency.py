...
def answer_rows(attempt: Optional[ExitTicketAttempt]) -> List[Dict]:
    ...
    Iterating the dict form yields string KEYS, so every caller that looped
    ``attempt.answers`` and called ``a.get(...)`` raised AttributeError and
    500'd the page. That is the whole reason this exists.

    Returns [] for anything else; drops non-dict entries. Summative attempts
    nest rows under answers['result']['per_question'] with different keys, but
    summatives have lesson=None so no lesson-scoped consumer reads them —
