...
def lesson_catalog(request):
    ...
    from ai_tutor.apps.curriculum.utils import parse_grade_level_string
    student_grade = ''
    ...
    for course in courses:
        ...
        from ai_tutor.apps.tutoring.competency_tracker import baseline_required_for, student_skills_snapshot
        ...
        if request.user.is_authenticated:
            try:
                from ai_tutor.apps.tutoring.models import ExitTicket as _ET, ExitTicketAttempt as _ETA
...
def generate_image(request):
    ...
    # Image safety check
    from ai_tutor.apps.safety import ImageSafetyFilter, SafetyAuditLog
...
def chat_start_session(request, lesson_id):
    ...
    if existing:
        ...
        from ai_tutor.apps.tutoring import simple_tutor
        if simple_tutor.is_enabled():
            from ai_tutor.apps.tutoring.simple_tutor.engine import start_for_view
            try:
...
def chat_respond(request, session_id):
    ...
    from ai_tutor.apps.tutoring.judges.safety import run_safety_judge
    from ai_tutor.apps.llm.models import ModelConfig as _MC
    from ai_tutor.apps.llm.client import get_llm_client as _get_client
    ...
    if safety_blocked:
        ...
        from ai_tutor.apps.safety import ContentFlag
        try:
...
def chat_answer_bank_question(request, session_id):
    ...
    from ai_tutor.apps.tutoring.bank_grader import (
...
def chat_exit_ticket(request, session_id):
    ...
    try:
        ...
        if _simple_tutor.is_enabled():
            from ai_tutor.apps.tutoring.simple_tutor.exit_ticket import (
                submit_exit_ticket as _simple_submit,
            )
            simple_resp = _simple_submit(session, answers)

            from ai_tutor.apps.tutoring.competency import attempt_response_block
            from ai_tutor.apps.tutoring.models import ExitTicket, StudentLessonProgress
...
def _try_add_participant(session, entry: dict, primary_institution) -> dict:
    ...
    from ai_tutor.apps.tutoring.models import SessionParticipant
    from ai_tutor.apps.accounts.models import StudentGroup, Institution
...
def lesson_competency(request, lesson_id):
    ...
    from ai_tutor.apps.tutoring.competency import competency_snapshot
...
def transcribe_audio(request, session_id):
    ...
    from ai_tutor.apps.safety import RateLimiter
...
def speak_text(request):
    ...
    from ai_tutor.apps.tutoring.audio_service import synthesize, synthesize_with_timestamps
...
def get_gamification_data(request):
    ...
    from ai_tutor.apps.tutoring.skills_models import StudentKnowledgeProfile, StudentAchievement, Achievement
...
def summative_take(request, course_id):
    ...
    from ai_tutor.apps.tutoring.summative_selection import select_questions_for_attempt
...
def summative_submit(request, course_id):
    ...
    from ai_tutor.apps.curriculum.models import Course
    from ai_tutor.apps.tutoring.models import ExitTicket, ExitTicketAttempt, ExitTicketQuestion
    from ai_tutor.apps.tutoring.summative_grading import grade_attempt
