...
from ai_tutor.apps.tutoring.models import TutorSession, SessionTurn, StudentLessonProgress
from ai_tutor.apps.tutoring.grader import check_math_answer, MathCheckResult
from ai_tutor.apps.tutoring.praise_filter import strip_praise_if_wrong
from ai_tutor.apps.tutoring.validator import validate_tutor_response
from ai_tutor.apps.tutoring.tracing import emit_span
...
# TUTOR_SYSTEM_PROMPT_TEMPLATE moved to apps/tutoring/prompts/anthropic.py
# (Phase 1 of task #229). Re-exported here so legacy imports keep working:
#   - apps/llm/prompts.py
...
Keep it to 1-2 sentences + question, ~60 words max.{no_reveal_clause}"""
    ...
    def start_review(self) -> TutorMessage:
        ...
        weak_skills = []
        try:
            from ai_tutor.apps.tutoring.personalization import RemediationService
...
Keep it to 2-3 sentences."""
    ...
    def respond(
        ...
        *,
        ...
        """
        from ai_tutor.apps.tutoring.tracing import start_span_buffer, reset_span_buffer
    ...
    def _respond_impl(
        ...
        *,
        ...
        if is_math_step:
            from ai_tutor.apps.tutoring.student_working_analyzer import analyze_working
            current_step = (
        ...
        try:
            ...
            if _legacy_leak_enabled and _bank_verdict_for_leak is False:
                ...
                from ai_tutor.apps.tutoring.answer_leak import detect_answer_leak
                from ai_tutor.apps.tutoring.validator import ISSUE_ANSWER_LEAK
        ...
        try:
            from ai_tutor.apps.tutoring.repeated_question import (
                detect_repeated_question, extract_questions,
                normalise_question_signature,
            )
            from ai_tutor.apps.tutoring.validator import ISSUE_REPEATED_QUESTION
            ...
            if _repeat_verdict is None and self.judge_client is not None:
                from ai_tutor.apps.tutoring.repeated_question import detect_template_repeat
                from ai_tutor.apps.tutoring.validator import ISSUE_SAME_TEMPLATE_REPEAT
        ...
        if validation.needs_regeneration:
            ...
            try:
                ...
                if _mech == 'self_retry':
                    from ai_tutor.apps.tutoring.regen.self_retry import summarise_self_retry
                    turn_metadata['regen_audit'] = summarise_self_retry(
                        ensemble_result,
                    )
                else:
                    from ai_tutor.apps.tutoring.regen import summarise_regen_cycles
    ...
    def _finalize_response(self, full_response: str, student_input: str, media: List[Dict]) -> Dict:
        ...
        if self.lesson.unit.course.is_math:
            from ai_tutor.apps.tutoring.llm_arithmetic_verifier import verify_arithmetic_claims
...
"""
    ...
    def _load_personalization(self):
        ...
        try:
            from ai_tutor.apps.tutoring.personalization import SessionPersonalizationService
...
IMPORTANT: Any question you ask must be complete and self-contained. Never say "which of these" or reference options/choices you haven't listed."""
    ...
    def _build_student_profile_block(self) -> str:
        ...
        try:
            from ai_tutor.apps.tutoring.skills_models import Skill, StudentSkillMastery, StudentKnowledgeProfile
            ...
            try:
                from ai_tutor.apps.tutoring.competency_tracker import student_skills_snapshot
                from ai_tutor.apps.curriculum.content_generator import combined_objectives_for_lesson
...
[/ENABLING OBJECTIVES]"""
    ...
    def _build_active_bank_question_block(self) -> str:
        ...
        from ai_tutor.apps.tutoring.question import Question as _Q
...
[/DIFFICULTY ADJUSTMENT]""")
    ...
    def _build_interleaved_practice_block(self) -> str:
        ...
        try:
            from ai_tutor.apps.tutoring.personalization import InterleavedPracticeService
...
Follow the current step; this concept will be covered in sequence."""
    ...
    def _build_system_prompt(self) -> str:
        ...
        from collections import defaultdict
        from ai_tutor.apps.llm.prompts import get_active_prompt_pack
        ...
        provider = (
            getattr(getattr(self, 'tutor_model_config', None), 'provider', '')
            or 'anthropic'
        )
        ...
        prompt_pack_override = (
            prompt_pack.tutor_system_prompt
            if (prompt_pack and prompt_pack.tutor_system_prompt and
        ...
        from ai_tutor.apps.llm.client import AnthropicClient as _AC
        ...
        if working_analysis is not None and working_analysis.steps:
            from ai_tutor.apps.tutoring.student_working_analyzer import (
    ...
    def _build_media_catalog(self) -> str:
        ...
        from ai_tutor.apps.llm.prompts import get_lesson_media
    ...
    def _build_question_bank_block(self) -> str:
        ...
        from ai_tutor.apps.tutoring.question_bank import (
    ...
    def _handle_pose_question_message(
        ...
        """
        from ai_tutor.apps.tutoring.question_bank import render_question_to_prose
    ...
    def _parse_question_signal(
        ...
        """
        from ai_tutor.apps.tutoring.question_bank import parse_question_signal
    ...
    def _get_or_commit_canonical(self, q, aa, kb_ctx):
        ...
        """
        from ai_tutor.apps.tutoring.canonical_committer import (
    ...
    def _grade_against_last_bank_question(self, student_input: str):
        ...
        """
        from ai_tutor.apps.tutoring.models import SessionTurn
        from ai_tutor.apps.tutoring.question import (
            Question as _Q,
            _DuckQuestion,
...
Which concept numbers were meaningfully covered?"""
    ...
    def _complete_session(self) -> TutorMessage:
        ...
        if not was_mastered:
            from ai_tutor.apps.tutoring.skills_models import StudentCompetencyRecord
    ...
    def _try_numeric_fast_path(
        ...
        if q_type == 'fill_in_blank':
            ...
            from ai_tutor.apps.tutoring.math_tools import safe_eval_expression
            correct_count = 0
            evaluated_count = 0
        ...
        if q_type == 'short_numeric':
            from ai_tutor.apps.tutoring.summative_grading import _math_norm
            import re as _re
            text = _math_norm(
    ...
    def _build_batch_grade_item(
        ...
        is_math flag.
        """
        from ai_tutor.apps.tutoring.exit_ticket_grader import build_batch_grade_item
        is_math = (
            self.lesson.unit.course.is_math
            if self.lesson.unit and self.lesson.unit.course
            else False
    ...
    def _complete_session_with_results(self, results: List[Dict], score: int) -> TutorMessage:
        ...
        try:
            ...
            if exit_ticket:
                ...
                try:
                    from ai_tutor.apps.tutoring.competency_tracker import refresh_student_snapshot
                    course = self.lesson.unit.course if self.lesson.unit else None
                    if course:
                        for participant in participants:
        ...
        try:
            ...
            from ai_tutor.apps.tutoring.achievements import check_and_award
    ...
    def _save_turn(self, role: str, content: str, metadata: Optional[Dict] = None):
        ...
        if role == "tutor":
            ...
            # ai_tutor.apps.tutoring.tracing and Phase 1 of
            # memory/agentic_platform_architecture_plan.md.
            from ai_tutor.apps.tutoring.tracing import flush_spans
            flush_spans(turn.id)
