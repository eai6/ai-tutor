...
class TestR1SkillExtractionPipeline(BaseTutoringTestCase):
    ...
    @patch('ai_tutor.apps.dashboard.background_tasks.connection')
    @patch('ai_tutor.apps.dashboard.background_tasks.generate_exit_ticket_for_lesson', return_value=0)
    @patch('ai_tutor.apps.tutoring.skill_extraction.SkillExtractionService')
    @patch('ai_tutor.apps.curriculum.content_generator.LessonContentGenerator')
    def test_pipeline_calls_skill_extraction(self, mock_gen_cls, mock_skill_svc_cls, mock_exit, mock_conn):
        ...
        from ai_tutor.apps.dashboard.background_tasks import generate_complete_lesson
