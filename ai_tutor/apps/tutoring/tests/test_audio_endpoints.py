...
class TestTranscribeAudioEndpoint(BaseTutoringTestCase):
    ...
    def _post(self, session_id, files=None, user=None):
        from ai_tutor.apps.tutoring.views import transcribe_audio
    ...
    @patch("ai_tutor.apps.safety.RateLimiter.check_rate_limit", return_value=(True, None))
    @patch("ai_tutor.apps.tutoring.audio_service.transcribe", return_value="Hello teacher")
    ...
    @patch("ai_tutor.apps.safety.RateLimiter.record_message")
    @patch("ai_tutor.apps.safety.RateLimiter.check_rate_limit", return_value=(True, None))
    @patch("ai_tutor.apps.tutoring.audio_service.transcribe", return_value=None)
    ...
    @patch("ai_tutor.apps.safety.RateLimiter.record_message")
    @patch("ai_tutor.apps.safety.RateLimiter.check_rate_limit", return_value=(False, "Too many requests"))
...
class TestSpeakTextEndpoint(BaseTutoringTestCase):
    ...
    def _post(self, body, user=None):
        from ai_tutor.apps.tutoring.views import speak_text
        request = self.factory.post(
            "/tutor/api/speak/",
    ...
    @patch("ai_tutor.apps.safety.RateLimiter.record_message")
    @patch("ai_tutor.apps.safety.RateLimiter.check_rate_limit", return_value=(True, None))
    @patch("ai_tutor.apps.tutoring.audio_service.synthesize_with_timestamps", return_value=None)
    @patch("ai_tutor.apps.tutoring.audio_service.synthesize")
    ...
    @patch("ai_tutor.apps.safety.RateLimiter.check_rate_limit", return_value=(True, None))
    @patch("ai_tutor.apps.tutoring.audio_service.synthesize_with_timestamps", return_value=None)
    @patch("ai_tutor.apps.tutoring.audio_service.synthesize", return_value=(None, "audio/wav"))
    def test_speak_tts_unavailable(self, mock_synth, mock_ts, mock_rate, mock_record):
        response = self._post({"text": "Hello"})
        self.assertEqual(response.status_code, 503)

    @patch("ai_tutor.apps.safety.RateLimiter.record_message")
    @patch("ai_tutor.apps.safety.RateLimiter.check_rate_limit", return_value=(False, "Rate limited"))
