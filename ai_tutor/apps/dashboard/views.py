...
def student_detail(request, student_id):
    ...
    from ai_tutor.apps.tutoring.competency import best_attempt, per_concept_breakdown
    courses_progress = {}
    ...
    from ai_tutor.apps.tutoring.models import ExitTicketAttempt as _ETAttempt
...
def course_detail(request, course_id):
    ...
    from ai_tutor.apps.dashboard.models import TeachingMaterialUpload, CurriculumUpload
...
def curriculum_upload(request):
    ...
    if request.method == 'POST':
        ...
        from ai_tutor.apps.dashboard.models import CurriculumUpload, TeachingMaterialUpload
...
def curriculum_approve(request, upload_id):
    ...
    try:
        ...
        if generate_content and total_lessons_for_gen > 0:
            ...
            from ai_tutor.apps.dashboard.background_tasks import run_async, generate_all_content_async
...
def lesson_session_report(request, lesson_id):
    ...
    """
    from ai_tutor.apps.curriculum.models import Lesson
    from ai_tutor.apps.tutoring.models import TutorSession, ExitTicketAttempt
    ...
    from ai_tutor.apps.curriculum.content_generator import combined_objectives_for_lesson
    ...
    from ai_tutor.apps.tutoring.competency import best_attempt
    objectives_data = []
...
def process_pending_materials(request, course_id):
    ...
    """
    from ai_tutor.apps.dashboard.models import TeachingMaterialUpload
    from ai_tutor.apps.dashboard.material_tasks import process_teaching_material, process_teaching_material_fast
    from ai_tutor.apps.dashboard.material_routing import should_dispatch_to_job
    from ai_tutor.apps.dashboard.background_tasks import run_async
...
def material_process(request, upload_id):
    ...
    if upload.status == 'pending_confirmation':
        from ai_tutor.apps.dashboard.cost_estimator import (
...
def course_upload_material(request, course_id):
    ...
    from ai_tutor.apps.dashboard.models import TeachingMaterialUpload
    from ai_tutor.apps.dashboard.material_tasks import process_teaching_material, process_teaching_material_fast
    from ai_tutor.apps.dashboard.material_routing import should_dispatch_to_job, MATERIAL_JOB_PAGE_THRESHOLD
    from ai_tutor.apps.dashboard.cost_estimator import estimate_material_cost
    from ai_tutor.apps.dashboard.background_tasks import run_async
    from ai_tutor.apps.dashboard.job_dispatch import dispatch_material_job
...
def settings_page(request):
        ...
        if action == 'general' and institution is not None and can_edit_school:
            ...
            if not email:
                ...
                if preferred_locale == '' or preferred_locale in dict(_settings.LANGUAGES):
                    from ai_tutor.apps.accounts.models import StudentProfile as _StudentProfile
    ...
    context = {
        **request.staff_ctx,
        ...
        # Staff page on main — intentionally not re-added here.
...
def exit_question_edit(request, question_id):
    ...
    from ai_tutor.apps.curriculum.quality_capture import (
    ...
    if 'plot_spec' in data or data.get('remove_plot_spec'):
        from ai_tutor.apps.tutoring.plot_spec import coerce_plot_spec
...
def course_regenerate_all(request, course_id):
    ...
    """
    from ai_tutor.apps.curriculum.models import Course, Lesson
    from ai_tutor.apps.dashboard.background_tasks import run_async, generate_complete_course
...
def lesson_regenerate(request, lesson_id):
    ...
    from ai_tutor.apps.curriculum.models import Lesson
    from ai_tutor.apps.dashboard.background_tasks import (
        run_async,
...
def staff_list(request):
        ...
        if action == 'invite_staff':
            ...
            if not inst:
                ...
                if email:
                    ...
                    from ai_tutor.apps.dashboard.models import PlatformConfig
                    try:
    ...
    if managed_ids is None:
        ...
        reach = Q(memberships__role='staff',
                  memberships__institution_id__in=managed_ids)
        if country is not None and request.staff_ctx.get('can_manage_country_team'):
            # A ministry colleague holds a CountryMembership and no Membership
            # at all, so a query that only knows about schools cannot see the
            # account this page just created.
            reach |= Q(country_memberships__country=country)
        candidates = User.objects.filter(reach, is_staff=False)
    ...
    for u in candidates:
        ...
        row = {
            ...
            'institutions': scope,
            'is_admin': u.is_staff,
            # A country account holds no Membership, so without this the
            # badge calls a ministry "Staff" beside a School column that
            # says the name of a country.
            'is_country': bool(country_ms),
...
def _exit_q_inplace_prompt_regen(question, teacher_guidance: str):
    ...
    from ai_tutor.apps.curriculum.content_regen.schemas import (
        FillInBlankRewrite, ShortAnswerRewrite, MatchingRewrite,
    )
    from ai_tutor.apps.curriculum.content_regen.prompt import (
...
def exit_question_inplace_regen(request, question_id):
    ...
    if qt == 'fill_in_blank':
        ...
        from ai_tutor.apps.curriculum.content_regen import run_fill_in_blank_regen
        judge_key = 'fill_in_blank'
        ...
        from ai_tutor.apps.curriculum.content_regen import run_short_answer_regen
        judge_key = 'short_answer'
        ...
        from ai_tutor.apps.curriculum.content_regen import run_matching_regen
        judge_key = 'matching'
...
def content_edit_events_list(request):
    ...
    from django.core.paginator import Paginator
    from ai_tutor.apps.curriculum.quality_models import ContentEditEvent, ContentEditTag
...
def content_edit_dashboard(request):
    ...
    from ai_tutor.apps.curriculum.quality_models import ContentEditEvent, ContentEditTag
    from ai_tutor.apps.curriculum.quality_metrics import (
...
def lesson_step_regenerate(request, step_id):
    ...
    try:
        if mode == 'auto_review':
            ...
            from ai_tutor.apps.curriculum.content_regen import run_step_regen
            from ai_tutor.apps.llm.models import ModelConfig
        ...
        from ai_tutor.apps.curriculum.content_regen import run_step_prompt_regen
    ...
    if request.method == 'POST':
        ...
        if action == 'upload_image':
            ...
            if uploaded:
                ...
                try:
                    from ai_tutor.apps.curriculum.figure_facts_extractor import (
...
def course_generate_media(request, course_id):
    ...
    """
    from ai_tutor.apps.dashboard.background_tasks import run_async, generate_media_async
    from ai_tutor.apps.dashboard.models import CurriculumUpload
...
def course_edit(request, course_id):
    ...
    if request.POST.get('action') == 'reparse':
        ...
        def _replan(upload_id, course_id):
            ...
            try:
                from ai_tutor.apps.dashboard.models import CurriculumUpload
                from ai_tutor.apps.curriculum.pipeline import generate_lesson_structure, complete_curriculum_upload
                from ai_tutor.apps.accounts.models import Institution
...
def lesson_live_monitor(request, lesson_id):
    ...
    from ai_tutor.apps.tutoring.models import TeacherGuidance
...
def _maybe_run_monitor_ai(lesson, session_data):
    ...
    from ai_tutor.apps.tutoring.models import TutorSession, TeacherGuidance
...
def feedback_submit(request):
    ...
    """
    from ai_tutor.apps.dashboard.models import FeedbackReport
...
def summative_review(request, course_id):
    ...
    from ai_tutor.apps.tutoring.summative_selection import coverage_report
    from ai_tutor.apps.accounts.models import Membership
...
def class_competency(request, course_id):
    ...
    exit tickets).
    """
    from ai_tutor.apps.tutoring.competency_tracker import (
...
def student_competency(request, course_id, student_id):
    ...
    from ai_tutor.apps.tutoring.competency_tracker import student_competency_table
    from django.contrib.auth.models import User as _User
