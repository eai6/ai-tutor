...
def _run_content_judges_for_steps(lesson, steps, *, force_model_config=None):
    ...
    from ai_tutor.apps.curriculum.content_judges.factual_step import (
        run_factual_step_judge,
    )
    from ai_tutor.apps.curriculum.content_judges.pedagogy_step import (
        run_pedagogy_step_judge,
    )
    from ai_tutor.apps.curriculum.content_judges.safety_content import (
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        for f in concurrent.futures.as_completed(futures):
            ...
            # Compose the multi-judge view of this step's verdicts so
            # we can decide regen on ANY judge REJECT (not just
            # factual_step). A judge that REJECTs but is `skipped` does
            # not count — only honest rejections trigger regen.
            judge_names = ('factual_step', 'pedagogy_step', 'safety_content')
            multi_verdicts = {
                jn: outputs[jn] for jn in judge_names if jn in outputs
            }
            any_failed = any(
                isinstance(v, dict)
                and not v.get('skipped')
                and not v.get('passed')
                for v in multi_verdicts.values()
            )
            all_skipped_or_passed = not any_failed

            # Default to whatever the judges said: all-pass/skipped →
            # auto_ok, otherwise → regen-then-decide. We may overwrite
            # verdict + status below if regen produces a clean
            # candidate across ALL judges.
            new_status = step.content_quality_status
            update_fields = ['judge_outputs', 'content_quality_status']

            if all_skipped_or_passed:
                ...
                # don't auto_flag — the judges say it's OK to ship,
                # just worth a teacher glance.
                new_status = LessonStep.ContentQualityStatus.AUTO_OK
            elif any_failed and regen_enabled:
                # Multi-judge regen: try to fix EVERY violation across
                # EVERY judge in one rewrite, re-judge all 3 per cycle,
                # accept only when ALL pass.
                regen_result = _regen_one_step(
                    step, lesson, multi_verdicts,
                )
                outputs['regen_audit'] = {
                    ...
                    'final_judge_results': dict(regen_result.final_judge_results or {}),
                    'cycles': regen_result.audit,
                }
                if regen_result.clean and regen_result.text:
                    # Replace the step text with the clean candidate
                    # and refresh ALL judge verdicts to the post-regen
                    # state so the UI shows the FINAL passing verdicts,
                    # not the original failing ones.
                    step.teacher_script = regen_result.text
                    new_status = LessonStep.ContentQualityStatus.AUTO_OK
                    for jn, jv in (regen_result.final_judge_results or {}).items():
                        if isinstance(jv, dict):
                            outputs[jn] = jv
                    update_fields.append('teacher_script')
                    regenerated += 1
                else:
                    # Regen exhausted without an all-judges-clean
                    # candidate — flag for human review. NEVER silently
                    # fail. Persist the partial post-regen verdicts
                    # too so the teacher sees the best attempt.
                    new_status = LessonStep.ContentQualityStatus.AUTO_FLAGGED
                    if regen_result.text:
                        step.teacher_script = regen_result.text
                        update_fields.append('teacher_script')
                    for jn, jv in (regen_result.final_judge_results or {}).items():
                        if isinstance(jv, dict):
                ...
                # Judges failed but regen disabled → flag directly.
                new_status = LessonStep.ContentQualityStatus.AUTO_FLAGGED
                flagged += 1
...
def _regen_one_step(step, lesson, judge_results: dict):
    """Wrapper around content_regen.run_step_regen that derives the
    needed lesson/step context from the LessonStep instance.

    `judge_results` is the multi-judge dict
    `{judge_name: verdict_dict}` covering factual_step / pedagogy_step
    / safety_content. Regen fixes the union of all violations and
    re-runs all 3 judges per cycle; `clean=True` only when ALL pass.
    ...
    """
    from ai_tutor.apps.curriculum.content_regen import run_step_regen, RegenResult
    try:
...
def _run_exit_question_judge_for_mcqs(lesson, mcqs, *, force_model_config=None):
    ...
    import concurrent.futures
    from django.conf import settings as _s
    from ai_tutor.apps.curriculum.content_judges.exit_question import (
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        for f in concurrent.futures.as_completed(futures):
            ...
            from ai_tutor.apps.tutoring.models import ExitTicketQuestion as _ETQ
            ...
            if verdict.skipped:
                ...
                if regen_result.clean and regen_result.question_text:
                    # Replace MCQ fields + refresh judge verdict.
                    q.question_text = regen_result.question_text
                    q.option_a = regen_result.option_a
                    q.option_b = regen_result.option_b
                    q.option_c = regen_result.option_c
                    q.option_d = regen_result.option_d
                    q.correct_answer = regen_result.correct_answer
    ...
    print(
        f"[ContentGen] [{lesson.title}] exit_question judge: "
        f"{persisted}/{len(mcqs)} persisted "
        f"(regenerated={regenerated}, auto_flagged={flagged})",
        flush=True,
    )
...
def _instructor_generate_exit_ticket(
    ...
    *,
    ...
    """
    from ai_tutor.apps.curriculum.content_gen_providers import (
        call_gen_structured_with_fallback,
        get_gen_provider_chain,
    )
    from ai_tutor.apps.curriculum.content_gen_schemas import GeneratedExitTicket
...
def _run_fill_in_blank_judge_for_qs(lesson, qs, *, force_model_config=None):
    ...
    import concurrent.futures
    from django.conf import settings as _s
    from ai_tutor.apps.curriculum.content_judges.fill_in_blank import (
...
def _regen_one_fib(q, lesson, verdict_dict: dict):
    ...
    from ai_tutor.apps.curriculum.content_regen import (
...
def _run_short_answer_judge_for_qs(lesson, qs, *, force_model_config=None):
    ...
    import concurrent.futures
    from django.conf import settings as _s
    from ai_tutor.apps.curriculum.content_judges.short_answer import (
...
def _run_matching_judge_for_qs(lesson, qs, *, force_model_config=None):
    ...
    import concurrent.futures
    from django.conf import settings as _s
    from ai_tutor.apps.curriculum.content_judges.matching import (
        run_matching_judge,
    )
...
def _regen_one_exit_q(q, lesson, verdict_dict: dict):
    ...
    Fail-soft: any exception inside regen returns a synthetic "no
    clean candidate" result so the caller still flags auto_flagged
    rather than crashing the pipeline.
    """
    from ai_tutor.apps.curriculum.content_regen import (
        run_exit_question_regen, ExitQuestionAutoRegenResult,
    )
    try:
        return run_exit_question_regen(
            original_question={
                'question_text': q.question_text or '',
        ...
        return ExitQuestionAutoRegenResult(
            question_text=q.question_text or '',
            option_a=q.option_a or '', option_b=q.option_b or '',
            option_c=q.option_c or '', option_d=q.option_d or '',
            correct_answer=q.correct_answer or '',
...
["Calculate the area of a rectangle given length and width", "Calculate the area of a triangle from its base and height", ...]"""
        ...
        try:
            ...
            from ai_tutor.apps.llm.models import ModelConfig
            from ai_tutor.apps.curriculum.content_gen_schemas import GranularSubSkillList
    ...
    def _generate_steps(self, lesson, curriculum_context: Dict) -> Dict:
        ...
        from ai_tutor.apps.curriculum.utils import format_grade_display
        unit = lesson.unit
        course = unit.course
...
11. The expected_answer MUST directly and completely answer the question as phrased.
   ...
   item, not just explain a comparison method"""
        ...
        from ai_tutor.apps.llm.prompts import get_prompt_or_default
        from ai_tutor.apps.curriculum.dok_framework import dok_guidance_for
        from ai_tutor.apps.curriculum.locale_prompts import locale_instruction_block
        ...
        if is_math:
            from ai_tutor.apps.curriculum.content_verifier import (
    ...
    def _save_steps_to_db(
        ...
        *,
        ...
        """
        from ai_tutor.apps.curriculum.models import LessonStep
        from ai_tutor.apps.curriculum.content_verifier import verify_lesson_step
        ...
        if is_math:
            ...
            from ai_tutor.apps.curriculum.models import Lesson  # noqa: F401
            existing = lesson.metadata or {}
            audit_blob = existing.get('verification_audit') or {}
            audit_blob['step_enabling_objective_validation'] = {
                **step_eo_stats,
...
def generate_exit_ticket_for_lesson(lesson, institution_id: int = None, force_regenerate: bool = False) -> Dict:
    ...
    from ai_tutor.apps.llm.models import ModelConfig
    from ai_tutor.apps.llm.client import get_llm_client
    from ai_tutor.apps.tutoring.management.commands.generate_exit_tickets import EXIT_TICKET_PROMPT
    ...
    if not granular_eos:
        from ai_tutor.apps.curriculum.models import LessonStep as _LessonStep
        seen_steps = set()
...
"""
    ...
    try:
        if is_math:
            from ai_tutor.apps.tutoring.management.commands.generate_exit_tickets import MATH_EXIT_TICKET_PROMPT
    ...
    try:
        ...
        from ai_tutor.apps.curriculum.mcq_distribution import (
