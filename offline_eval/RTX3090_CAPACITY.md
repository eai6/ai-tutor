...
**Answer.** Concurrent students, by turn budget:
...
platform gap" below before planning against them.** A real turn on the same
...
**A turn is TIMED, not derived.** Each simulated student issues call 1 (the
...
An earlier version of this document estimated the turn as `2 x one call`. That
...
turn is 20.0s; at N=64 it predicted 26.2s against 39.4s. The capacity figures
it produced were 30-40% too generous.

Measured from the traces, 95% of 27B turns and 100% of 4B turns really are two
calls, so timing the pair is right for almost every turn. The 5% single-call
minority makes turns slightly *cheaper*, so this is conservative.
...
- **Students in a lesson** — larger than N, because a student spends most of a
  turn reading and typing, using no GPU. Derived, and it inherits a think-time
  assumption, so it is a range and never one figure.

---

## Full results — whole turns, measured
...
The benchmark issues only model calls. A real turn also runs the platform's own
...
| | value |
|---|---|
| benchmark turn at N=1 | 8.9s |
| `math_27b_v3` board turn, 641 real turns | **14.9s** |
| unmodelled platform work | **~6.0s per turn** |
...
resolve it, instrument a production turn end-to-end under concurrency rather
...
```bash
# whole turns (call1 + call2), which is what a student waits
...
Raw data: `offline_eval/sweep_rtx3090_turnmode.log` (whole turns, authoritative)
...
- **A turn is not two identical calls.** See the Method section.
